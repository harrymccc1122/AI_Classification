import os
import time

import pandas as pd

import matplotlib.pyplot as plt

import h5py

import classifier
import hdf5_maker


def plot_person_data(df: pd.DataFrame):
    data_jump = df.loc[(df["category"] == 0)]
    data_walk = df.loc[(df["category"] == 1)]

    # Create a 3D plot
    fig = plt.figure()
    gs = fig.add_gridspec(2,2, wspace=0.5, hspace=0.5)
    ax = fig.add_subplot(gs[:, 0], projection='3d')

    jumping, = ax.plot(data_jump['Linear Acceleration x (m/s^2)'], data_jump['Linear Acceleration y (m/s^2)'], data_jump['Linear Acceleration z (m/s^2)'], color='r')
    jumping.set_label("Jumping")
    walking, = ax.plot(data_walk['Linear Acceleration x (m/s^2)'], data_walk['Linear Acceleration y (m/s^2)'], data_walk['Linear Acceleration z (m/s^2)'], color='g')
    walking.set_label("Walking")

    ax.legend()
    # Set labels and title
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Directional acceleration')

    ax2 = fig.add_subplot(gs[0, 1])
    ax2.plot(data_jump["Time (s)"], data_jump["Absolute acceleration (m/s^2)"], color='r')
    ax2.set_ylim([-5, 100])
    ax2.set_xlabel('Time (s)')
    ax2.set_ylabel('Abs. accel. (m/s^2)')
    ax2.set_title('Jumping abs. acceleration')

    ax3 = fig.add_subplot(gs[1, 1])
    ax3.plot(data_walk["Time (s)"], data_walk["Absolute acceleration (m/s^2)"], color='g')
    ax3.set_ylim([-5, 100])
    ax3.set_xlabel('Time (s)')
    ax3.set_ylabel('Abs. accel. (m/s^2)')
    ax3.set_title('Walking abs. acceleration')

    # Show plot
    plt.show()


def visualize(hdf5_filename):
    person_data = hdf5_maker.read_hdf5_original_datasets(hdf5_filename)
    # get all the samples
    train, test = hdf5_maker.read_hdf5_train_test(hdf5_filename)
    train += test
    # get the features
    features_df = classifier.extract_features(train)

    # # plot acceleration vs time
    # for df in person_data:
    #     plot_person_data(df)
    #
    # # plot histogram of features
    # ax = features_df.plot.hist(bins=20, alpha=0.5)
    # ax.plot()
    # plt.show()

    ax = features_df.loc[(features_df['category'] == 0) & (features_df['person'] == 1)]['meanA'].plot.hist(bins=15, alpha=0.5)
    ax.plot()
    ax = features_df.loc[(features_df['category'] == 1) & (features_df['person'] == 1)]['meanA'].plot.hist(bins=15, alpha=0.5)
    ax.plot()
    ax.set_title("jumping vs walking for person 1")
    plt.show()

    ax = features_df.loc[(features_df['category'] == 1) & (features_df['person'] == 1)]['meanA'].plot.hist(bins=15, alpha=0.5)
    ax.plot()
    ax.set_title("walking")
    plt.show()


def main():
    visualize("data.h5")


if __name__ == "__main__":
    main()